...
import { getUserFacingErrorMessage, isUserRejectedError, readSafeStackFile, WALLET_CONNECTION_UNAVAILABLE_MESSAGE } from '../src/app/userFacingErrors.js'
...
describe('user-facing errors', () => {
	test('normalizes wallet rejections, provider objects, and unknown thrown values', () => {
		...
		assert.equal(getUserFacingErrorMessage(new WalletRequestTimeoutError('eth_chainId')), WALLET_CONNECTION_UNAVAILABLE_MESSAGE)
		assert.equal(getUserFacingErrorMessage(new WalletRequestTimeoutError('eth_getCode')), 'The wallet did not respond to eth_getCode. Reconnect the wallet and try again.')
	})
